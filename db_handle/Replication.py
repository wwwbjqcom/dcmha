...
class _rollback:
    ...
    table_filed = None
    filed_table_name = None
    filed_database_name = None
    filed_gtid = None
...
class Echo(object):
    ...
    def TractionHeader(self, thread_id, database_name, sql_statement, timestamp, _pos):
        ...
        if _remote_filed._thread_id:
            if _remote_filed._thread_id == thread_id:
                ...
                print '{}  GTID_NEXT : {} at pos : {}'.format(_remote_filed._tid_gid_time, _remote_filed._tid_gid,
                                                              _remote_filed._tid_gid_pos)
    ...
    def TractionVlues(self, before_value=None, after_value=None, type=None):
        if _remote_filed._thread_id:
            ...
            if _rollback.table_filed:
                if _rollback.filed_table_name == _rollback.table and _rollback.filed_database_name == _rollback.database:
                    _rollback._myfunc.CreateSQL(before_value=before_value, after_value=after_value, event_type=type)
            else:
                _rollback._myfunc.CreateSQL(before_value=before_value, after_value=after_value, event_type=type)
        else:
...
class GetRollStatement(object):
    ...
    def DeleteEvent(self, values):
        sql = 'insert into {}.{}({}) values('.format(_rollback.database, _rollback.table,
                                                     ','.join([a[0] for a in self.column_list]))
...
class Read(Echo):
    ...
    def read_table_map_event(self, event_length):
        ...
        for idex in range(len(colums_type_id_list)):
            if colums_type_id_list[idex] in [column_type_dict.MYSQL_TYPE_VAR_STRING,
                                             column_type_dict.MYSQL_TYPE_VARCHAR]:
                metadata = self.read_uint16()
                metadata_dict[idex] = 2 if metadata > 255 else 1
                bytes += 2
            elif colums_type_id_list[idex] in [column_type_dict.MYSQL_TYPE_BLOB,
                                               column_type_dict.MYSQL_TYPE_MEDIUM_BLOB,
                ...
                bytes += 1
                                               ...
                                               column_type_dict.MYSQL_TYPE_DATETIME2,
                                               column_type_dict.MYSQL_TYPE_TIME2]:
                metadata = self.read_uint8()
                metadata_dict[idex] = metadata
                bytes += 1
        ...
        if self.__packet is None:
            self.file_data.seek(
                event_length - binlog_event_header_len - table_map_event_fix_length - 5 - database_name_length
    ...
    def read_row_event(self, event_length, colums_type_id_list, metadata_dict, type):
        ...
        while event_length - bytes > binlog_quer_event_stern:
            ...
            for idex in range(len(colums_type_id_list)):
                ...
                elif colums_type_id_list[idex] in [column_type_dict.MYSQL_TYPE_DOUBLE,
                                                   column_type_dict.MYSQL_TYPE_FLOAT]:
                ...
                elif colums_type_id_list[idex] in [column_type_dict.MYSQL_TYPE_VARCHAR,
                                                   column_type_dict.MYSQL_TYPE_VAR_STRING,
                                                   column_type_dict.MYSQL_TYPE_BLOB,
                                                   column_type_dict.MYSQL_TYPE_TINY_BLOB,
                                                   column_type_dict.MYSQL_TYPE_LONG_BLOB,
                                                   column_type_dict.MYSQL_TYPE_MEDIUM_BLOB]:
...
class CheckEvent(Echo):
    def __init__(self, filename=None, start_position=None, stop_position=None,
                 ...
                 user=None, host=None, passwd=None, port=None, r_table=None, r_database=None):
        ...
        if rollback:
            if user is None or host is None or passwd is None:
                Usage()
                sys.exit()
            if r_table and r_database:
                _rollback.filed_table_name, _rollback.filed_database_name = r_table, r_database
                _rollback.table_filed = True
    ...
    def UnPack(self, pack):
        ...
        if unpack[1] != 0:
            ...
            if event_type == binlog_events.QUERY_EVENT:
                ...
                database_name, table_name, self.cloums_type_id_list, self.metadata_dict = _Read.read_table_map_event(
                    event_length)
                self.Tablemap(execute_time, table_name)
...
def main(argv):
    _argv = {}
    try:
        opts, args = getopt.getopt(argv[1:], 'hrf:t:g:H:p:P:u:',
                                   ['help', 'file=', 'start-position=', 'stop-position=', 'start-datetime=',
                                    'stop-datetime=', 'host=', 'user=', 'passwd=', 'port=', 'thread-id=', 'gtid=',
                                    'rollback', 'remote', 'log-file=', 'block', 'server-id=', 'database=', 'table='])
    ...
    if 'rollback' in _argv:
        if 'remote' in _argv:
            ...
            CheckEvent(filename=_argv['file'], gtid=(_argv['gtid'] if 'gtid' in _argv else None),
                       ...
                       _thread_id=(_argv['thread-id'] if 'thread-id' in _argv else None),
                       r_database=(_argv['database'] if 'database' in _argv else None),
                       r_table=(_argv['table'] if 'table' in _argv else None))
        elif 'gtid' in _argv:
